...
<strong>전통적 프로그래밍:</strong> 데이터 + 규칙 → 답
...
<div class="example-box">
<strong>📌 이 책에서 배울 것:</strong>
1~6장: 머신러닝 (사이킷런 sklearn)
7~9장: 딥러닝 (케라스 Keras / 파이토치 PyTorch)
10장: 언어 모델 (트랜스포머, LLM)
</div>`,
...
<div class="highlight-box">
<strong>접속:</strong> colab.research.google.com
<strong>장점:</strong> 설치 불필요, 구글 드라이브 연동, 무료 GPU/TPU
</div>

<h2>텍스트 셀 vs 코드 셀</h2>
<div class="example-box">
<strong>📝 텍스트 셀:</strong> 마크다운 형식으로 설명 작성
<strong>⚡ 코드 셀:</strong> 파이썬 코드 작성 및 실행 (Shift+Enter)
</div>

<h2>노트북 기본 단축키</h2>
<div class="example-box">
<strong>Shift + Enter:</strong> 셀 실행 후 다음 셀로
<strong>Ctrl + Enter:</strong> 셀 실행 (이동 없음)
<strong>Ctrl + M B:</strong> 아래에 새 코드 셀 추가
<strong>Ctrl + M D:</strong> 셀 삭제
</div>

<div class="warning-box">
<strong>⚠️ 코랩 주의사항:</strong>
• 90분 미사용 시 세션 초기화 (변수 사라짐)
• 최대 12시간 연속 사용 후 초기화
• 데이터는 구글 드라이브에 저장 권장
</div>`,
...
<div class="highlight-box">
<strong>사이킷런 KNN 분류 3단계:</strong>
1. 모델 생성: KNeighborsClassifier(n_neighbors=5)
2. 훈련:      model.fit(X_train, y_train)
3. 예측:      model.predict([[무게, 길이]])
</div>

<div class="example-box">
<strong>[문제해결 과정] 도미와 빙어 분류:</strong>
① 데이터 준비 (리스트로 만들기)
...
<div class="warning-box">
<strong>⚠️ 이 장의 문제점:</strong>
훈련 데이터 전체로 정확도를 평가 → 과적합 가능성
...
<h2>지도 학습과 비지도 학습</h2>
<div class="example-box">
<strong>지도 학습(Supervised):</strong> 정답(레이블)이 있는 데이터로 학습
예: 도미/빙어 분류, 주가 예측

<strong>비지도 학습(Unsupervised):</strong> 정답 없이 데이터의 패턴을 스스로 발견
예: 군집화(클러스터링), 차원 축소
</div>
...
<div class="highlight-box">
<strong>일반적인 분리 비율:</strong>
훈련 세트: 70~80% | 테스트 세트: 20~30%
...
<div class="example-box">
<strong>넘파이 기본 활용:</strong>
np.array() — 리스트를 NumPy 배열로
arr.shape — 배열 크기 확인
arr[:, 0] — 열(column) 선택
</div>`,
...
<div class="warning-box">
<strong>⚠️ 스케일 문제:</strong>
무게: 0~1000g, 길이: 10~50cm → 무게 단위가 거리를 지배
...
<div class="warning-box">
<strong>⚠️ 중요 원칙:</strong>
테스트 세트에는 반드시 <strong>훈련 세트의 통계(μ, σ)</strong>를 사용해야 합니다.
...
<div class="example-box">
<strong>R² = 1 − (타깃-예측)² 합 / (타깃-평균)² 합</strong>
...
<div class="example-box">
<strong>과대적합 vs 과소적합:</strong>
훈련 R² ≫ 테스트 R²  → <strong>과대적합</strong> (overfitting)
...
<div class="warning-box">
<strong>⚠️ KNN 한계:</strong>
길이 100cm 생선 → 이웃 최대 길이 50cm → 50cm의 이웃 평균만 반환
...
<div class="highlight-box">
<strong>사이킷런 선형 회귀:</strong>
from sklearn.linear_model import LinearRegression
model = LinearRegression()
model.fit(X_train, y_train)
print(model.coef_)      # 기울기(a)
...
<div class="example-box">
<strong>PolynomialFeatures(degree=2, include_bias=False):</strong>
특성 [a, b] → [a, b, a², ab, b²]
5개 특성 → 15개 특성으로 확장
</div>
...
<div class="highlight-box">
<strong>릿지(Ridge) 회귀 — L2 규제:</strong>
손실 = MSE + α × Σcoef²
계수를 작게 만들되 완전히 0으로 만들지 않음
...
<div class="highlight-box">
<strong>라쏘(Lasso) 회귀 — L1 규제:</strong>
손실 = MSE + α × Σ|coef|
일부 계수를 완전히 0으로 만들어 <strong>특성 선택</strong> 효과
...
<div class="example-box">
<strong>C 파라미터 (규제 강도의 역수):</strong>
C ↑ → 규제 약화 (기본값 C=1.0)
C ↓ → 규제 강화
</div>`,
...
<div class="example-box">
<strong>배치 경사 하강법:</strong> 전체 데이터로 한 번에 학습 (느리지만 안정)
<strong>미니배치 경사 하강법:</strong> 일부씩 나눠 학습 (균형)
...
<div class="highlight-box">
<strong>사이킷런 SGDClassifier:</strong>
from sklearn.linear_model import SGDClassifier
sgd = SGDClassifier(loss='log_loss', max_iter=1)
...
<div class="example-box">
<strong>에포크와 과대/과소적합:</strong>
에포크 너무 적음 → <strong>과소적합</strong> (훈련 덜 됨)
에포크 너무 많음 → <strong>과대적합</strong> (훈련 데이터 암기)
→ 검증 손실이 최소인 에포크에서 조기 종료(Early Stopping)!
</div>

<div class="warning-box">
<strong>loss 매개변수 주요 값:</strong>
'hinge': SVM 기반, 클래스 확률 미제공
'log_loss': 로지스틱 회귀, 확률 제공
...
<div class="highlight-box">
<strong>지니 불순도(Gini Impurity):</strong>
gini = 1 − Σ(pᵢ²)
클래스가 완전히 섞이면 0.5, 완전히 분리되면 0

<strong>엔트로피(Entropy):</strong>
entropy = −Σ(pᵢ × log₂(pᵢ))
</div>

<div class="example-box">
<strong>사이킷런 결정 트리:</strong>
...
<div class="example-box">
<strong>사이킷런 GridSearchCV:</strong>
from sklearn.model_selection import GridSearchCV
params = {'max_depth': [2,3,4], 'min_samples_leaf': [1,2,3]}
...
<div class="highlight-box">
<strong>from sklearn.ensemble import RandomForestClassifier</strong>
...
<div class="example-box">
<strong>히스토그램 기반 그레이디언트 부스팅:</strong>
...
<div class="example-box">
<strong>정형 vs 비정형 데이터:</strong>
정형(표 데이터): 앙상블(랜덤 포레스트, XGBoost) 강세
...
<div class="example-box">
<strong>비지도 학습 활용 사례:</strong>
• 고객 세분화 (비슷한 구매 패턴끼리 묶기)
• 이상 탐지 (정상 패턴에서 벗어난 것 탐지)
• 차원 축소 (특성 수 줄이기)
</div>
...
<div class="example-box">
<strong>axis 매개변수:</strong>
axis=0: 행 방향(샘플 간) 평균 → 대표 이미지
...
<div class="highlight-box">
<strong>K-평균 동작 과정:</strong>
1. 무작위로 K개의 중심점 초기화
2. 각 샘플을 가장 가까운 중심으로 배정
3. 각 클러스터의 새 중심 = 소속 샘플들의 평균
4. 중심이 더 이상 변하지 않을 때까지 2~3 반복
</div>

<div class="example-box">
<strong>사이킷런 KMeans:</strong>
from sklearn.cluster import KMeans
km = KMeans(n_clusters=3, random_state=42)
km.fit(fruits_2d)
print(km.labels_)         # 각 샘플의 클러스터 번호
print(km.cluster_centers_)  # 클러스터 중심 (shape: 3×10000)
</div>

<h2>최적의 K 찾기 — 엘보우 방법</h2>
<div class="example-box">
<strong>이너샤(Inertia) = 클러스터 내 거리 합계</strong>

K를 늘릴수록 이너샤 감소, 하지만 어느 지점에서 감소폭이 꺾임
...
<div class="highlight-box">
<strong>사이킷런 PCA:</strong>
from sklearn.decomposition import PCA
pca = PCA(n_components=50)  # 50개 주성분
pca.fit(fruits_2d)
fruits_pca = pca.transform(fruits_2d)  # (300, 50)으로 축소
...
<div class="example-box">
<strong>pca.explained_variance_ratio_:</strong>
각 주성분이 원본 분산의 몇 %를 설명하는지
...
<div class="example-box">
<strong>PCA 활용:</strong>
• 10,000 → 50 차원 축소 후 KMeans 적용 → 성능 동일, 속도 200배
...
&nbsp;&nbsp;keras.layers.Dense(300, activation='relu'),  # 1번째 은닉층
...
<div class="highlight-box">
<strong>ReLU(x) = max(0, x)</strong>

시그모이드: 양쪽 끝에서 기울기 → 0 (역전파 시 소실)
ReLU: x > 0 이면 기울기 = 1 (소실 없음) → 은닉층 기본값
</div>

<h2>옵티마이저 (Optimizer)</h2>
<div class="example-box">
<strong>주요 옵티마이저:</strong>
SGD: 기본 경사 하강법
Adam: 적응적 학습률 (가장 많이 사용)
RMSprop: 과거 기울기 제곱의 지수 평균
...
<div class="highlight-box">
keras.layers.Dropout(0.3)  # 30% 뉴런 무작위 제거

→ 모델이 특정 뉴런에 의존하지 않도록 강제
→ 예측 시에는 모든 뉴런 사용 (자동 처리)
</div>

<h2>콜백 (Callback)</h2>
<div class="example-box">
<strong>ModelCheckpoint:</strong> 최고 성능 모델 자동 저장
<strong>EarlyStopping:</strong> 검증 손실이 개선 없으면 조기 종료
...
<div class="highlight-box">
<strong>합성곱 연산:</strong>
입력 이미지의 일부(3×3)와 필터(3×3)의 원소별 곱의 합
→ 특성 맵(Feature Map) 생성
...
<div class="example-box">
<strong>padding 매개변수:</strong>
'same': 입력과 동일한 크기 유지 (가장자리에 0 패딩)
...
<h2>CNN 전체 구조</h2>
<div class="highlight-box">
입력(28×28×1) → Conv+ReLU → MaxPool → Conv+ReLU → MaxPool
→ Flatten → Dense → Dense(출력)

앞부분(Conv+Pool): 이미지 특성 추출기
뒷부분(Dense): 분류기
</div>`,
...
<div class="highlight-box">
<strong>BatchNormalization:</strong>
각 배치의 활성화값을 정규화 → 학습 안정화 + 속도 향상

keras.layers.BatchNormalization()  # Conv2D 다음에 삽입
</div>

<div class="example-box">
<strong>Dense 모델 vs CNN 성능 비교:</strong>
Dense(Flatten): 패션 MNIST 정확도 ~87%
CNN:            패션 MNIST 정확도 ~92%
...
<div class="example-box">
<strong>가중치 추출:</strong>
conv_weights = model.layers[0].weights[0].numpy()
# shape: (3, 3, 1, 32) — 3×3 필터 32개

plt.imshow(conv_weights[:,:,0,0], vmin=-1, vmax=1)
plt.colorbar()
</div>
...
<div class="example-box">
keras.layers.SimpleRNN(8, return_sequences=True)  # 모든 시점 출력
keras.layers.SimpleRNN(8)                          # 마지막 시점만 출력
</div>

<div class="warning-box">
<strong>⚠️ 기울기 소실 문제:</strong>
시퀀스가 길어질수록 먼 과거 정보가 희미해짐
→ LSTM, GRU로 해결
</div>`,
...
<div class="highlight-box">
<strong>Embedding 층:</strong>
keras.layers.Embedding(300, 16)  # 300단어 사전, 16차원 임베딩

원-핫 인코딩: "사랑" → [0,0,1,0,...] (300차원, 희소)
...
<div class="highlight-box">
<strong>LSTM의 3개 게이트:</strong>
• 망각 게이트(f_t): 오래된 정보를 얼마나 지울지
• 입력 게이트(i_t): 새 정보를 얼마나 저장할지
• 출력 게이트(o_t): 은닉 상태로 얼마나 내보낼지
...
model = keras.Sequential([
&nbsp;&nbsp;keras.layers.Embedding(300, 16),
&nbsp;&nbsp;keras.layers.LSTM(32, dropout=0.3, return_sequences=True),
...
<div class="example-box">
<strong>SimpleRNN vs LSTM vs GRU 비교:</strong>
SimpleRNN: 파라미터 적지만 긴 시퀀스에 약함
LSTM: 긴 의존성 강점, 파라미터 많음
...
<div class="highlight-box">
<strong>셀프 어텐션 (Self-Attention):</strong>
Attention(Q, K, V) = softmax(QK^T / √d_k) · V

같은 시퀀스 내 단어끼리의 관계를 학습
"그것이 맛있었다" → "그것"이 "음식"을 참조
</div>

<h2>트랜스포머 구조</h2>
<div class="example-box">
<strong>인코더 블록 (N회 반복):</strong>
LayerNorm → Multi-Head Self-Attention → Residual
LayerNorm → Feed-Forward Network → Residual

<strong>위치 인코딩 (Positional Encoding):</strong>
...
<div class="highlight-box">
<strong>허깅페이스 (Hugging Face):</strong>
...
<h2>토큰 디코딩 전략</h2>
<div class="highlight-box">
<strong>그리디(Greedy):</strong> 매 스텝 가장 높은 확률 토큰 선택 (단순, 반복 많음)
<strong>Beam Search:</strong> 상위 k개 후보를 병렬 탐색 (균형)
<strong>Top-k Sampling:</strong> 상위 k개 중 무작위 선택 (다양성)
