    ...
    lessons: [
      {
        ...
        duration: "15분",
        codeExampleIds: ["cv-install"],
        content: `
<h2>영상처리(Image Processing)란?</h2>
<p>디지털 영상을 <strong>입력</strong>으로 받아 다른 영상이나 정보를 <strong>출력</strong>하는 기술입니다.</p>

<div class="highlight-box">
<strong>영상처리의 3가지 수준:</strong>
🔹 <strong>저수준(Low):</strong> 화소 연산 — 밝기 변환, 필터링, 엣지 검출
🔹 <strong>중수준(Mid):</strong> 영역 분할 — 객체 검출, 모폴로지
🔹 <strong>고수준(High):</strong> 이해·인식 — 얼굴 인식, 번호판 인식
</div>

<h3>영상처리의 역사</h3>
<div class="example-box">
1960년대: NASA 우주탐사 위성 사진 복원
1970년대: 의료 영상(CT, MRI) 처리
1980년대: 산업용 기계 비전
1990년대~: 멀티미디어·인터넷 확산
...
<h3>관련 분야</h3>
<div class="example-box">
컴퓨터 비전(Computer Vision) — 기계가 영상을 이해
패턴 인식(Pattern Recognition) — 규칙에서 패턴 검출
...
<div class="highlight-box">
<strong>영상 크기 계산:</strong>
가로 W × 세로 H × 채널 C = 전체 데이터 크기
예: 640×480 컬러 영상 = 640 × 480 × 3 = 921,600 바이트 ≈ 900KB
</div>

<h3>픽셀 깊이(Bit Depth)</h3>
<div class="example-box">
1비트 영상: 흑백 2가지 (0 또는 1)
8비트 영상: 256 단계 (0~255) → 그레이스케일
24비트 영상: RGB 각 8비트 → 약 1,677만 색상
32비트 영상: RGBA (투명도 포함)
</div>

<h3>영상처리 응용 분야</h3>
<div class="example-box">
🏥 의료: MRI·CT·X-ray 분석, 종양 검출
🚗 자동차: 자율주행, 차선 인식, 번호판 인식
🔒 보안: 얼굴 인식, 지문 인식, CCTV 분석
🏭 제조: 불량품 검출, 치수 측정
📡 위성: 지형 분석, 기상 관측
</div>`,
    ...
    lessons: [
      {
        ...
        duration: "15분",
        codeExampleIds: ["cv-install", "cv-ch02-src"],
        content: `
<h2>OpenCV란?</h2>
...
<div class="highlight-box">
<strong>OpenCV 주요 특징:</strong>
• 무료 오픈소스 (BSD 라이선스)
• C++ 기반, Python 인터페이스 제공
• 실시간 처리 최적화
• 딥러닝(DNN) 모듈 포함
...
<h3>OpenCV 주요 모듈</h3>
<div class="example-box">
core — 기본 배열 연산, 데이터 구조
imgproc — 영상 처리 (필터, 변환, 히스토그램)
highgui — 윈도우 표시, 이벤트 처리
videoio — 비디오 캡처/저장
...
2. PyCharm 설치 (jetbrains.com/pycharm)
3. pip로 OpenCV 설치
</div>

<h3>OpenCV 설치</h3>
<div class="example-box">
<strong># 기본 버전 (CPU만)</strong>
pip install opencv-python

<strong># 추가 모듈 포함 (SIFT 등)</strong>
pip install opencv-contrib-python

<strong># 함께 설치할 패키지</strong>
...
<div class="example-box">
import cv2
print(cv2.__version__)  # 예: 4.8.1
import numpy as np
print(np.__version__)
</div>

<div class="warning-box">
<strong>⚠️ OpenCV-Python 주의:</strong>
• cv2.imread()는 BGR 채널 순서 사용 (RGB 아님!)
    ...
    lessons: [
      {
        ...
        duration: "20분",
        codeExampleIds: ["cv-numpy-basics", "cv-ch03-src"],
        content: `
<h2>영상처리에서 주로 쓰는 자료 구조</h2>

<div class="example-box">
<strong>리스트 (List):</strong> 화소 좌표, 컬러 값
point = [100, 200]       # (x, y) 좌표
color = [255, 0, 0]      # BGR 색상

<strong>튜플 (Tuple):</strong> 불변 좌표·크기
size  = (640, 480)       # 영상 크기
roi   = (x, y, w, h)     # 관심 영역
</div>
...
<div class="highlight-box">
<strong>img[y1:y2, x1:x2]</strong> — 관심 영역(ROI) 추출

roi = img[100:200, 50:150]   # y축 100~200, x축 50~150
gray = img[:, :, 0]           # 첫 번째 채널만
flipped = img[::-1, :]        # 상하 반전
</div>
...
<div class="highlight-box">
<strong>영상 → NumPy 배열 구조:</strong>
컬러 영상: (H, W, 3)  — H×W 픽셀, BGR 3채널
그레이스케일: (H, W)   — H×W 픽셀, 1채널
타입: uint8 (0~255)
</div>

<h3>자주 쓰는 NumPy 함수</h3>
<div class="example-box">
np.zeros((480, 640, 3), dtype=np.uint8)  # 검정 영상
np.ones((480, 640), dtype=np.uint8) * 128  # 회색 영상
img.shape   → (480, 640, 3)    # (H, W, C)
img.dtype   → uint8            # 픽셀 타입
img.copy()  → 독립 복사 (중요!)
img.T       → 전치 (축 교환)
</div>
    ...
    lessons: [
      {
        ...
        duration: "20분",
        codeExampleIds: ["cv-interface", "cv-ch04-src"],
        content: `
<h2>윈도우 제어</h2>
<div class="example-box">
cv2.namedWindow('창 이름', cv2.WINDOW_NORMAL)  # 창 생성
cv2.imshow('창 이름', img)                     # 영상 표시
...
<h3>그리기 함수</h3>
<div class="example-box">
cv2.line(img, (x1,y1), (x2,y2), (B,G,R), 두께)
cv2.rectangle(img, (x1,y1), (x2,y2), color, 두께)
cv2.circle(img, (cx,cy), 반지름, color, 두께)
cv2.putText(img, '텍스트', (x,y), 폰트, 크기, color)
      ...
      },
      {
        ...
        duration: "20분",
        codeExampleIds: ["cv-video", "cv-ch04-src"],
        content: `
<h2>영상 파일 처리</h2>
<div class="highlight-box">
<strong>읽기 플래그:</strong>
cv2.IMREAD_COLOR    = 1  → 컬러(BGR, 기본값)
...
<h2>기본 배열 처리</h2>
<div class="example-box">
cv2.add(src1, src2)      # 포화 덧셈 (결과 0~255 클리핑)
cv2.subtract(src1, src2) # 포화 뺄셈
cv2.multiply(src1, src2) # 원소별 곱
cv2.divide(src1, src2)   # 원소별 나눗셈
...
<div class="example-box">
cv2.mean(img)              # 채널별 평균
cv2.meanStdDev(img)        # 평균 + 표준편차
cv2.minMaxLoc(img)         # 최솟값, 최댓값, 위치
...
<h3>행렬 연산</h3>
<div class="example-box">
cv2.gemm(A, B, alpha, C, beta)  # 일반 행렬 곱
    ...
    lessons: [
      {
        ...
        duration: "20분",
        codeExampleIds: ["cv-pixel-ops", "cv-ch06-src"],
        content: `
<h2>화소(Pixel) 직접 접근</h2>
<div class="example-box">
# 단일 픽셀 읽기
b, g, r = img[y, x]       # 컬러 (BGR)
val = gray[y, x]           # 그레이스케일

# 픽셀 쓰기
img[y, x] = [0, 255, 0]    # 초록 픽셀
gray[y, x] = 128           # 중간 밝기
</div>
...
<div class="highlight-box">
<strong>가감 연산:</strong>
brighter = cv2.add(img, np.ones_like(img)*50)  # 밝게
darker   = cv2.subtract(img, np.ones_like(img)*50)  # 어둡게

<strong>명암 대비 조절:</strong>
contrast = img.astype(float) * 1.5   # 대비 증가
...
<div class="highlight-box">
<strong>목적:</strong> 히스토그램을 전체 0~255 범위에 고르게 분포시켜 대비를 향상

equ = cv2.equalizeHist(gray)

# CLAHE: 지역적 평활화 (더 자연스러움)
...
<h3>컬러 공간별 특징</h3>
<div class="example-box">
<strong>BGR/RGB:</strong> 기본 색상 표현, 화소 처리
<strong>HSV/HSI:</strong> Hue(색상)·Sat(채도)·Val(명도) — 색상 기반 검출
<strong>Lab:</strong> 인간 시각에 균일한 색차 → 색상 비교
...
sharpened = cv2.filter2D(img, -1, lap_kernel)
        ...
        diagramType: "cv-chap07-filter",
        codeExampleId: "cv-filtering",
      },
      {
        id: "cv7-2", title: "7.2 에지 검출",
        duration: "20분",
        codeExampleIds: ["cv-edge-detection", "cv-ch07-src"],
        content: `
<h2>에지(Edge)란?</h2>
...
<div class="highlight-box">
<strong>Canny 4단계:</strong>
① 가우시안 블러 (노이즈 제거)
② Sobel 그래디언트 계산
③ Non-Maximum Suppression (에지 얇게)
④ 이중 임계값 (강한 에지·약한 에지 선별)
...
lap_abs = cv2.convertScaleAbs(lap)
        ...
        diagramType: "cv-chap07-edge",
        codeExampleId: "cv-edge-detection",
      },
      {
        id: "cv7-3", title: "7.3~7.4 기타 필터링과 모폴로지",
        duration: "20분",
        codeExampleIds: ["cv-morphology", "cv-ch07-src"],
        content: `
<h2>모폴로지 연산 (Morphology)</h2>
<p>이진(binary) 영상에서 형태를 변형시키는 연산입니다. 구조 요소(커널)의 형태로 팽창·침식 방향을 결정합니다.</p>

<div class="highlight-box">
<strong>구조 요소 생성:</strong>
kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5,5))
# MORPH_RECT / MORPH_ELLIPSE / MORPH_CROSS
</div>

<div class="example-box">
<strong>침식 (Erosion):</strong> 밝은 영역 축소, 노이즈 제거
eroded = cv2.erode(binary, kernel)

<strong>팽창 (Dilation):</strong> 밝은 영역 확장, 구멍 메우기
dilated = cv2.dilate(binary, kernel)

<strong>열림 (Opening = 침식→팽창):</strong> 작은 노이즈 제거
opened = cv2.morphologyEx(img, cv2.MORPH_OPEN, kernel)

<strong>닫힘 (Closing = 팽창→침식):</strong> 작은 구멍 메우기
closed = cv2.morphologyEx(img, cv2.MORPH_CLOSE, kernel)
</div>`,
        keyPoints: ["침식: 밝은 영역 축소 → 잡음 제거", "팽창: 밝은 영역 확장 → 구멍 채우기", "열림=침식→팽창(노이즈), 닫힘=팽창→침식(구멍)"],
        diagramType: "cv-chap07-morph",
    ...
    lessons: [
      {
        ...
        duration: "20분",
        codeExampleIds: ["cv-geometric", "cv-ch08-src"],
        content: `
<h2>사상(Mapping)이란?</h2>
...
<h3>크기 변경 (확대/축소)</h3>
<div class="highlight-box">
cv2.resize(img, dsize=(640, 480))          # 절대 크기
cv2.resize(img, None, fx=0.5, fy=0.5)     # 비율 변경

<strong>보간법 선택:</strong>
cv2.INTER_NEAREST  — 최근접(빠름, 계단 현상)
cv2.INTER_LINEAR   — 양선형(기본, 균형)
cv2.INTER_CUBIC    — 3차 회선(고품질, 느림)
cv2.INTER_AREA     — 축소에 최적
</div>

<h3>보간법의 차이</h3>
<div class="example-box">
<strong>최근접 이웃(Nearest Neighbor):</strong>
가장 가까운 픽셀 값을 그대로 사용 → 빠르지만 계단 현상

<strong>양선형(Bilinear):</strong>
...
<div class="highlight-box">
<strong>3점 대응으로 어파인 행렬 계산:</strong>
    ...
    lessons: [
      {
        ...
        duration: "20분",
        codeExampleIds: ["cv-fft", "cv-ch09-src"],
        content: `
<h2>공간 주파수란?</h2>
<p>영상에서 밝기가 <strong>얼마나 빠르게 변하는가</strong>를 나타냅니다.</p>

<div class="example-box">
<strong>저주파(Low Frequency):</strong>
밝기 변화가 완만한 영역 (배경, 평평한 부분)

<strong>고주파(High Frequency):</strong>
...
idct = cv2.idct(dct_compressed)    # 역변환 (복원)
  ...
  {
    id: 'cv10', partId: 2, chapId: 'chap10',
...
<div class="example-box">
<strong>워핑:</strong> 제어점을 이용해 영상을 변형 (얼굴 특징점 이동)
<strong>모핑:</strong> 두 영상 사이를 자연스럽게 전환
모핑 = 워핑 + 크로스 페이드(addWeighted)
  ...
  {
    id: 'cv11', partId: 3, chapId: 'chap11',
    title: "Ch 11 — 응용 사례 I",
    ...
    lessons: [
      {
        ...
        duration: "25분",
        codeExampleIds: ["cv-paint-app", "cv-ch11-src"],
        content: `
<h2>그림판 프로그램 전체 구조</h2>

<div class="highlight-box">
<strong>구현 컴포넌트:</strong>
① 캔버스 영역 (흰 배경 영상)
② 팔레트 영역 (색상 선택)
③ 아이콘 영역 (도구 선택: 펜, 선, 사각형, 원)
④ 마우스 이벤트 처리
</div>
...
<h3>성별 분류 개요</h3>
<div class="highlight-box">
<strong>히스토그램 비교 방식:</strong>
① 얼굴 영역 히스토그램 계산
② 남성/여성 평균 히스토그램과 비교
③ cv2.compareHist()로 유사도 계산
④ 가장 유사한 클래스로 분류
</div>`,
  ...
  {
    id: 'cv12', partId: 3, chapId: 'chap12',
    ...
    lessons: [
      {
        ...
        duration: "30분",
        codeExampleIds: ["cv-coin-detect", "cv-ch12-src"],
        content: `
<h2>동전 인식 전체 처리 과정</h2>

<div class="highlight-box">
<strong>처리 파이프라인:</strong>
① 영상 캡처 → 전처리 (그레이스케일, 블러)
② 허프 원 검출 → 동전 객체 검출
③ 개별 동전 ROI 영상 생성
④ 색상 히스토그램 계산 (HSV 공간)
...
for cx, cy, r in circles[0]:
    ...
    roi_masked = cv2.bitwise_and(roi, roi, mask=mask)
        ...
        diagramType: "cv-chap12",
        codeExampleId: "cv-coin-detect",
      },
      {
        id: "cv12-2", title: "12.2~12.3 번호판 SVM 검출 및 k-NN 인식",
        duration: "35분",
        codeExampleIds: ["cv-plate-detect", "cv-ch12-src"],
        content: `
<h2>번호판 검출 (SVM)</h2>

<div class="highlight-box">
<strong>번호판 검출 전체 흐름:</strong>
① 에지 검출 + 윤곽선 추출
② 번호판 후보 영역 추출 (가로:세로 비율 필터)
③ SVM 모델로 번호판 여부 판별
④ 번호판 영역 정렬 (어파인 변환)
</div>
