...
import { llmModules, LLM_PART_LABELS } from '../data/curriculum_llm';

const DIFFICULTY_LABELS = ['', '입문', '초급', '중급', '고급', '심화'];
const DIFFICULTY_COLORS = ['', 'text-green-600', 'text-blue-600', 'text-yellow-600', 'text-orange-600', 'text-red-600'];
const DIFFICULTY_BG     = ['', 'bg-green-100', 'bg-blue-100', 'bg-yellow-100', 'bg-orange-100', 'bg-red-100'];
...
const partColors = {
  ...
  3: { bg:'bg-red-50',    border:'border-red-300',    header:'bg-red-700',    bar:'bg-red-500',    light:'bg-red-100',    text:'text-red-700'    },
  4: { bg:'bg-green-50',  border:'border-green-300',  header:'bg-green-700',  bar:'bg-green-500',  light:'bg-green-100',  text:'text-green-700'  },
  5: { bg:'bg-cyan-50',   border:'border-cyan-300',   header:'bg-cyan-700',   bar:'bg-cyan-500',   light:'bg-cyan-100',   text:'text-cyan-700'   },
  6: { bg:'bg-indigo-50', border:'border-indigo-300', header:'bg-indigo-700', bar:'bg-indigo-500', light:'bg-indigo-100', text:'text-indigo-700' },
  7: { bg:'bg-orange-50', border:'border-orange-300', header:'bg-orange-700', bar:'bg-orange-500', light:'bg-orange-100', text:'text-orange-700' },
};
...
const colorMap = {
  ...
  yellow: { bg:'bg-yellow-50', border:'border-yellow-300', header:'bg-yellow-600', text:'text-yellow-700', light:'bg-yellow-100' },
};
...
export default function LlmCurriculumPage({ getModuleProgress, isLessonCompleted, isQuizCompleted }) {
  ...
  const groupedByPart = llmModules.reduce((acc, m) => {
  ...
  return (
    <div className="animate-fade-in space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-gray-800">LLM 원리와 실전 구현</h1>
        <p className="text-gray-500 mt-1">
          토큰화부터 트랜스포머, RLHF, RAG, 에이전트, 멀티모달까지 — 7 PART 18 챕터 심화 커리큘럼
        </p>
        ...
        return (
          <div key={partId} className="space-y-4">
            <div className={`rounded-xl ${pc.header} text-white px-5 py-3`}>
              <div className="font-bold text-sm">{LLM_PART_LABELS[parseInt(partId)]}</div>
            </div>
